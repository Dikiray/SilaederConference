...
def login(request):
  ...
  if request.method == 'POST':
    if form.is_valid():
      ...
      form = LoginForm()
      text = 'Неверное имя пользователя или пароль'
  return render(request, 'silsite/login.html', {'form': form, 'text': text, 'alert': False})

@login_required(redirect_field_name='')
def new_project(request):
  n = Project.objects.aggregate(Max('id'))['id__max']
  m = 0 if (n == None) else n + 1
  prj = Project(teacher=request.user, name=f'Проект регистрируется#{m}')
  prj.save()
...
def edit_project(request, id):
  ...
  if request.method == 'POST':
    ...
    if form.is_valid():
      ...
      prj.direction = request.POST['project_direction']
      try:
        if request.POST['time'] == '':
           prj.time = prj.time
        else:
      	   prj.time = request.POST['time']
      except:
        prj.time = prj.time
      if request.FILES.get('presentation') != None: #request.FILES.get('short_text') != None and request.FILES.get('text') != None and request.FILES.get('presentation') != None:
        #prj.short_text = request.FILES['short_text']
        #prj.text = request.FILES['text']
        prj.presentation = request.FILES['presentation']
      else:
        prj.presentation = prj.presentation
     # else:
       # form = ProjectForm(instance=prj)
        #text_ = 'Please, upload files'
        #return render(request, 'silsite/edit_project.html', {'form': form, 'text_': text_, 'short_text': prj.short_text.name})
      prj.save()
      return render(request, 'silsite/login.html', {'form': form, 'text': '', 'alert': True})
    else:
...
def get_conf_projects(request, id):
  prj = Project.objects.all()
  prj_sorted = []
  for i in prj:
      if i.id > id:
          prj_sorted.append(i)
  return render(request, 'silsite/main_page.html', {'projects': prj_sorted, 'user': request.user})
...
def set_default(request):
  ...
  for i in prj:
      i.time=9
      i.save()
  return(render(request,'silsite/index.html'))
